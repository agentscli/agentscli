...
Every other tool you've probably used already decided, on your behalf, what's worth spending your attention on watching. A permission prompt here, a collapsed tool-call summary there - conveniences that also mean you never had to look closely at a single turn to trust the next ten. That's the problem this whole module exists to fix, stated as plainly as it gets: **you cannot budget what you have never watched run once.** Before you can decide which of another agent's guardrails you'd actually rebuild, you need to have seen, one time, exactly what happens with none of them in place.
