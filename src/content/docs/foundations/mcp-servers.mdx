...
That's the gap the **Model Context Protocol** (MCP) closes. MCP is an open wire format for connecting an AI tool to an external service — a database, a browser, a ticket tracker, a file store, a search index. An MCP *server* is a typed adapter that exposes one of those systems as a set of tools (e.g. `jira.search_issues`, `postgres.query`, `figma.get_node`); the CLI is the *client* that discovers them at startup and lets the model call them as if they were built in. Now *the agent itself* runs `jira.get_ticket("AUTH-412")`, sees the result, and acts on it — the integration moves from "you, by hand" to "the agent, in-flow."
...
Every tool a server exposes registers a schema in the model's context window at startup, whether or not it's ever called. Toggle servers on and off and watch what they cost before you've typed a single message — then flip on schema deferral to see why it exists.
