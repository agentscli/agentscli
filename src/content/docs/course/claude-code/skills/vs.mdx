...
description: "Three ways to package work: a skill the agent invokes on recognition, a command you trigger, a subagent with its own context. Match the tool to the job."
sidebar:
...
- **Let the agent invoke it** when running it at the wrong moment is *harmless* - checks, summaries, lookups, regenerating something. Keep `finish-pr` command-only because this example writes `CHANGELOG.md` and may run formatting fixes: add `disable-model-invocation: true`, or split the read-only checks from the mutating steps.
