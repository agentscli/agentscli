...
Any of these means you're trusting the picture instead of the code it claims to describe:

- A diagram appeared with zero tool calls in the transcript before it.
- "That edge looks right" instead of a file-and-line receipt for it.
- No date, no owner, and no regeneration path on anything committed to shared docs.
- The prompt asked to "diagram the codebase" instead of naming one question.
