...
description: "Your first hour with OpenCode - from nothing installed to one real, reviewed change shipped in a Go project you just inherited and don't yet trust."
---
...
And there's one detail that can earn the skeptic's attention early: `feedmill` is typed Go, and OpenCode can read the same LSP diagnostics your editor turns into red squiggles. There's a setup step the skeptic should know up front, though - LSP is disabled by default, so you turn it on in your config, and you bring your own Go toolchain and `gopls`. OpenCode auto-detects `.go` files by extension and, once LSP is on, starts `gopls` for you. Make sure `gopls` is on your `PATH` before debugging the OpenCode configuration. See the [LSP documentation](https://opencode.ai/docs/lsp/) for the current server behavior. With that in place, a proposed change isn't guessing about your types - it's looking at the same `gopls` output you are. That's the difference between an agent that *hopes* the code compiles and one that *checks*. OpenCode documents several built-in LSP servers, Go among them. You'll feel it for the first time when you ship the date fix, and lean on it in every chapter after.
