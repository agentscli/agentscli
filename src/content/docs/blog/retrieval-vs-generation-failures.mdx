...
def on_mcp_tool_result(tool_name, query, result):
    log.info("retrieval", extra={
        ...
        "payload_hash": hashlib.sha256(
            result.get("text", "").encode()
        ).hexdigest()[:12],
    })
    return result
```

Log stable chunk identifiers and hashes on every tool call before the model touches the result. Keep raw queries and retrieved text out of routine production logs: runbook payloads can contain incident details, customer data, or credentials. For a controlled evaluation corpus, store the raw payload in an access-controlled trace with an explicit retention period, or replay the logged chunk IDs from the source when you investigate. Now the wrong answer stops being a mystery without turning observability into a second data leak. Either the retry-policy chunk is among the retrieved IDs or it is not, and that one visible fact is the difference between fixing your index and wordsmithing a prompt that cannot reach the bug. **You cannot debug a context window you cannot see, but you also should not copy the whole window into an unrestricted log.**
