...
| Session resume/fork | `-c`/`--continue`, `-r`/`--resume <id>`, `--fork-session` (all headless) | `codex resume`, `codex fork` | Sessions w/ undo-redo | `/resume` (CLI) | — | — (*Unverified*) |
...
| First-party CI integration | Claude Agent SDK | GitHub Action | GitHub / GitLab integrations | Bugbot (PR reviewer) | GitHub Actions, Coding Agent, PR review | `pi -p` scripted into any CI runner |
| Programmatic SDK | Claude Agent SDK | Codex SDK | OpenCode SDK | Cloud Agents REST API | REST API (*Unverified* on stable path) | Pi SDK + RPC mode |
