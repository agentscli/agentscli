...
reader who picked Pi on one page finds it missing on the next.

`PermissionSim` is the worst case: it renders **three** panels (Claude Code,
Codex, OpenCode) at the top of `permissions.mdx`, still encoding the tool scope
the authoring guide carried before Cursor, Copilot, and Pi were integrated. It
is the most visible remaining artifact of that drift.

None of these were fixed in the 2026-07-26 pass, because filling them needs real
per-tool permission, headless, and model facts that nobody verified against
upstream docs - and inventing them is worse than leaving the gap visible. They
are tracked in the foundations staleness register with the project's research
notes. Anyone doing the next fact-verification sweep should fix the widget data
in the same pass as the chapter text.

**Other structural debt from the same audit:**

- `PrimitivePicker` and `PluginPacker` both call `useWidgetFrame('ppk-root')`
  and both ship a full stylesheet under `.ppk-*`. The only duplicated prefix in
  the directory. Latent while they never share a page; rename one.
- `SubagentFanout` has no `-data.ts`; its facts are inline in the component,
  against the file convention above.
- Ten widgets declare `role="tablist"` with no roving tabindex, no `tabIndex`,
  and no `role="tabpanel"`, against the keyboard contract above. One shared
  tab-row component would fix all ten.
- `command-expander-data.ts` contains a live en dash, which renders to the page
  even though every `.mdx` grep is clean. Repo-wide dash sweeps miss data files.
