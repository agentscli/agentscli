    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: 22
          cache: npm
      - run: npm install -g @openai/codex@x.y.z   # pin the version you tested; check npm for the latest
...
Two details in that workflow earn their keep, because an unattended run lives or dies on reproducibility. First, **pin the Codex version** - `@openai/codex@x.y.z` rather than a bare `@openai/codex`. Installing `latest` on every push means a release you never tested can change the run's behaviour mid-week; pinning to the version you verified keeps the check stable, and you bump the pin deliberately, on your own schedule. (The `x.y.z` is a placeholder - run `npm view @openai/codex version` to find the current one and pin that.) Second, the **`actions/setup-node` step with `cache: npm`** caches the downloaded packages between runs, so the install isn't re-fetching the world on every push - faster runs, and one fewer thing that can fail on a network blip.
