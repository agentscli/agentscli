...
Make the rounding contract visible at that interface, not implicit in four callers:

| Decision | Contract at `pricing.ts` |
|---|---|
| Internal arithmetic | Keep full precision until the named boundary |
| Tax rounding | Round half up to currency minor units per line item |
| Display rounding | Format the already-rounded minor-unit amount; never recompute tax |
| Caller override | Pass an explicit rounding mode only where the domain permits it |

The exact policy will differ by product and jurisdiction. The design point is that callers can see which boundary owns rounding and tests can pin it in one place.
