...
description: "A second agent, spawned after the first, scoped to read-only tools, that decomposes a builder's self-report into falsifiable claims and checks each one against the real data — and because it can't chat back, the only way to fix what it misses is to edit its rules."
sidebar:
...
You already have everything the verifier needs, for free: every Pi session persists its full, uncompacted history to a JSONL file on disk, regardless of what got summarized into context along the way. Point the verifier at that file instead of at the builder's closing message, and it isn't reading a summary — it's reading exactly what happened, tool call by tool call:

```text
pi --mode json -p "Read the builder's session log at
  .pi/sessions/reextract-batch-07.jsonl. It claims to have re-extracted
...
That decomposition belongs in a durable file, not a one-off prompt — put it in the verifier's own system prompt (`.pi/agents/verifier.md`, or handed in via `--append-system-prompt`), the same way `.pi/SYSTEM.md` captures `stash`'s conventions so you stop re-typing them every session. And that's not incidental — it's the whole reason this pattern is worth building at all.
