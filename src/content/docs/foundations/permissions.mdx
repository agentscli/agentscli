...
Every CLI in scope encodes exactly that three-way decision — *do it without asking*, *ask me first*, or *refuse outright* — on every action the agent attempts. The shapes are wildly different:
...
- **Cursor:** auto-run modes (Auto-review / Allowlist / Run Everything) plus command allow/denylists; OS-level Agent Sandbox on macOS, Linux, and Windows (via WSL2), folded into Allowlist as a layer rather than its own mode.
...
| OS-level sandbox | No | Yes (`read-only` / `workspace-write` / `danger-full-access`) | No (wrap in a container if you need it) | Yes (Agent Sandbox: macOS/Linux native, Windows via WSL2) | Optional (`chat.agent.sandbox.enabled`, macOS/Linux) | Via container/extension, not built-in |
| Approval modes | Named modes: `default` / `acceptEdits` / `plan` / `dontAsk` / `bypassPermissions` (Shift+Tab to cycle), over allow/ask/deny rules | `untrusted` / `on-request` / `never` | `allow` / `ask` / `deny` | Auto-review / Allowlist / Run Everything | `default` / `autoApprove` / `autopilot` | None — YOLO by default; gates come from a container or extension |
