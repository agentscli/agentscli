...
description: "The approval policy decides which actions pause for your y/n: untrusted asks about almost everything, on-request lets the agent stop only for risky moves, never runs end to end."
sidebar:
...
Typing `-a` every invocation gets old. The persistent equivalent is `approval_policy` in `config.toml`, which takes the same three values (plus a granular form for finer control — see the [config reference](https://developers.openai.com/codex/config-reference)):
