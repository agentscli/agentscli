...
Heads-up on which Postgres server you wire in: Anthropic's old reference `@modelcontextprotocol/server-postgres` is **deprecated and archived**. Do not copy an archived server into production without reviewing its source and security history. The example above uses `postgres-mcp` ("Postgres MCP Pro", by Crystal DBA); pin the version you deploy, review its upstream security posture, and keep its access mode restricted to read-only.
...
:::tip[Try it]
Add an MCP server to your `opencode.json` config. Ask a question that requires it. Then remove the server and ask the same question - notice what the agent loses. Check the context cost with the server mounted.
:::
