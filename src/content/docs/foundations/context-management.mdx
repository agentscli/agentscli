...
The failure mode above, compressed into eleven steps. Walk through a session and watch the window fill: the fixed overhead you pay before typing, the exploration spikes, a subagent detour that keeps 45k of noise out entirely, the auto-compact threshold — and what `/compact` actually keeps. Token counts are illustrative; the shape is the point.
