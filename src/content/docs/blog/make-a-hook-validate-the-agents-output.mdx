...
```python
# .claude/hooks/validate_sql.py - PreToolUse, before the query runs
...
try:
    ...
    used = extract_columns(sql)        # your tested SQL parser
    unknown = used - cols
except (KeyError, TypeError, ValueError, sqlite3.Error) as error:
    print(f"SQL validation unavailable: {error}", file=sys.stderr)
    sys.exit(2)
...
One detail in that snippet earns its keep: a known invalid call prints the documented `PreToolUse` deny envelope and exits `0`, so the permission-decision reason returns to Claude as correction context. A validator failure prints to stderr and exits `2`, which blocks the tool call rather than silently treating a broken checker as success. Test both paths against the installed Claude Code version; the current [hooks reference](https://code.claude.com/docs/en/hooks) owns the event envelope and exit-code behavior. The loop has a name in agent research, where it goes by **self-correction** or the **reflection** pattern: catch a wrong output, tell the model what's wrong, let it try again. What separates this instance from that family comes next. The ground truth is deterministic, the retry is capped, and the rate that reaches production is provably zero, which an open-ended reflection loop left to its own judgment never guarantees.
