...
description: "A workspace-level server is open to every conversation. A custom agent can carry its own MCP instead, so library-reviewer gets the registry and nothing else."
sidebar:
