...
Cursor belongs with the first class in practice and with a caveat on paper: Cursor documents Plan Mode as a research-and-plan workflow, while its public docs do not publish an exhaustive tool allowlist. Treat it as workflow guidance, not as a security boundary. See the [Plan Mode announcement](https://cursor.com/blog/plan-mode) and [agent modes documentation](https://docs.cursor.com/en/agent/modes).
