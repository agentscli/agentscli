...
:::caution[The test]
If you cannot name the input boundary, the action gate, the execution boundary, and the deterministic postcondition for a workflow, it is not ready for unattended use.
:::
...
This is why “plan mode,” “auto,” and “deny” need qualification. They are product labels, not universal security guarantees. **The label is not the boundary; the layer is.** See [Permissions & sandboxing](/foundations/permissions/), [Hooks](/foundations/hooks/), and [Plan mode](/foundations/plan-mode/) for the tool-specific boundaries.
