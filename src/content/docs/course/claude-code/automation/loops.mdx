...
description: "A PR review only fires when someone opens a PR. The nightly job is the other half: a headless call on a schedule, fenced so an unwatched run can't do damage."
sidebar:
    ...
    steps:
      ...
      - uses: anthropics/claude-code-action@v1
        with:
          ...
          claude_args: "--max-turns 15 --permission-mode dontAsk --allowedTools Read,Bash(npm test),Bash(git status *),Bash(gh issue create *)"
```
...
- **Fail closed on permissions.** This is the [permissions lesson](/course/claude-code/automation/permissions/) cashed in. `dontAsk` plus a tight `--allowedTools` means the run can execute the suite and open an issue, and *refuse everything else by default* - there's no human to wave through the thing you didn't anticipate. Deny rules add another layer while permission checks are active; they do not make bypass mode safe.
