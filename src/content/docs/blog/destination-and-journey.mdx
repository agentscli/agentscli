...
For the per-tool mechanics, see [Rules](/foundations/rules/) for the requirements doc as committed context, [Plan mode](/foundations/plan-mode/) for proposing the build order before any code, and [Subagents](/foundations/subagents/) for the isolated-context pattern each phase run mirrors.
