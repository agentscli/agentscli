...
A **subagent** is an isolated worker the main agent spawns to do a discrete task - it runs its own loop in a fresh context window that doesn't become part of yours. The parent normally receives a distilled result; some clients also keep the worker thread or detailed activity inspectable.
...
<YouTubeEmbed id="JMSHrHuBfEg" title="Subagents" />
...
<StackDiagram
  panels={[
    { title: 'parent context', rows: [
      { label: 'rules, skills, conversation, the work you actually came here to do' },
      { label: 'you delegate a task' },
      { label: '"the bug is in session.ts:142, token mutated by middleware"' },
    ]},
    { title: 'subagent context', rows: [
      { label: '(fresh window)' },
      { label: 'system prompt' },
      { label: "subagent's instructions" },
      { label: 'task: "debug the flaky test in auth/session.ts"' },
      { label: 'reads 17 files' },
      { label: 'runs the test 6 times' },
      { label: 'tries 4 hypotheses' },
      { label: 'returns ONE sentence' },
      { label: '(window discarded)' },
    ]},
  ]}
/>

The parent normally receives the useful result rather than the worker's full transcript. Whatever the subagent learned to *find* the answer may remain inspectable in the client, but it does not automatically expand the parent's working context. Whatever you needed to *act* on the answer is still there, uncluttered.
...
One delegation keeps a window clean; fan-out is where subagents change what's *possible*. The same audit - "find dead code across `api/`, `ui/`, and `tests/`" - run both ways. Token counts are illustrative; toggle and watch your window.

<SubagentFanout client:visible />
<noscript><p class="widget-noscript">This fan-out example needs JavaScript. The <a href="#comparison">comparison table</a> below remains available without it.</p></noscript>
...
|---|---|---|---|---|---|---|
| Built-in subagents | `Explore`, `Plan`, `general-purpose` | `default`, `worker`, `explorer` | `general`, `explore`, `scout` | None by default | `explore`, `task`, `general-purpose`, and others vary by surface | None |
...
- **"Agent" in OpenCode** - usually means a *primary agent* (`build`, `plan`) you switch into with Tab. "Subagent" is reserved for a worker the primary spawns.
- **"Agent" in Claude Code** - colloquially means a subagent, unless someone says "agent team."
- **"Custom agent" in Copilot** - one `.agent.md` file, dual-purpose: a *persona* you pick from the chat dropdown, or a subagent another agent delegates to via `runSubagent`. Two invocation paths, not two separate primitives.
