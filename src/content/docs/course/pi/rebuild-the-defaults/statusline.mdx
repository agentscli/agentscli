...
description: "Pi's TUI already ships a footer - cwd, session, token usage, cost, context percentage, model. Build your own from ctx.ui.setFooter to swap in what you actually want to track: a running tool tally, reconstructed from session history so it survives forks the same way tilldone's state did."
sidebar:
