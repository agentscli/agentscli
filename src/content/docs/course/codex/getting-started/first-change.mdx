...
It **read** the relevant files first - including the existing test, so its new test would match the house style instead of inventing one. It **proposed** a plan in two sentences before touching anything. Then it **stopped and asked**. That pause is the leash. Codex's default **Auto** posture doesn't stop for every edit - it works inside your project directory on its own and pauses when it judges a move consequential, which an edit on the date-parsing path of a repo holding your real ledger squarely is. (How much it asks is a dial you'll set on purpose in the [approvals chapter](/course/codex/approvals-sandbox/approval-modes/).) You approve the importer edit, it moves on to the test, and then does the thing that actually closes the ticket - it runs the test itself:
