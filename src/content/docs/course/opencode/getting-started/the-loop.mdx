...
description: "Read, propose, approve, apply — the four beats under every OpenCode task. Watch them once from the read-only plan agent, on a harmless question, so the real fix in the next lesson holds no surprises."
sidebar:
