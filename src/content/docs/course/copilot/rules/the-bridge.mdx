...
- **`CLAUDE.md`** - enabled by the relevant current Copilot/VS Code setting where supported. Copilot looks in documented repository and personal locations; the exact paths are surface-specific.
- **`GEMINI.md`** - supported by current Copilot instruction discovery on several surfaces; check the support matrix before relying on it in an IDE.
...
- If you only ever use Copilot in this repo, `.github/copilot-instructions.md` is the most broadly supported repository-wide choice, with surface-specific limitations.
