...
:::tip[Try it]
Spawn a child Pi with a read-only task: "list all functions in stash without docstrings." Watch the JSONL events stream by on stdout, then parse the output and extract just the final summary from the `agent_end` event. Notice how the child's context stays entirely separate from yours - nothing it read leaks back into your session.
:::
