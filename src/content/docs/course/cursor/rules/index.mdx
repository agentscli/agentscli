...
description: "Stop Cursor reintroducing the same bugs by writing the project's standing facts down once. Learn the four rule layers, the .mdc frontmatter that maps to the four rule types, AGENTS.md as the portable plain-markdown path Cursor reads natively, the legacy .cursorrules file, and the 'globbed rule only attaches when the file enters the agent's context' sharp edge - a Cursor 2.0 change that makes a rule fire less reliably than you'd think."
---
