...
description: "One default-exported function, no build step, and two folders Pi checks on startup. That's the whole surface area you need before writing your first line of extension code."
sidebar:
...
{/* TODO widget: PrimitivePicker would be a natural fit for "extension vs. skill" here, but it's a fact-bound cross-tool widget reserved for foundations pages per WIDGETS.md's course rule - skills haven't been introduced yet in this course anyway (that's module 7). Revisit if an evergreen equivalent gets built. */}
