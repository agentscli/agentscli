...
| **You'll have** | Reviewable output on a fresh branch or PR, depending on the surface, sized to one review sitting, plus a spec and running log that get better every run |
| **Costs** | One watched run to certify the setup. After that, minimal attention between kickoff and the notification |
...
<Aside type="tip" title="In a hurry?">
  ...
  3. Reviewable output on a fresh branch or PR. Never push to a protected branch.
...
2. **Detach, land reviewable output, and come back only on notification.** The entry point varies - chat UI, CLI flag, an assigned issue, an `@mention` - and so does the execution model. A cloud run can continue after you close your laptop; a local or remote process still depends on its host staying available. Output may be a PR or a commit on a working branch, depending on the surface. Never push to anything protected. Auto-merge is an opt-in escalation, not a default.
...
| Codex | Codex cloud | ChatGPT Codex UI, IDE "cloud delegation," `@codex` in a GitHub **PR** comment (starts a cloud task); `@codex review` on a PR is the separate review trigger. Issue-style kickoff is documented for Linear, not GitHub Issues | opens a GitHub PR, or pull the diff down locally instead | GitHub notification on the issue/PR; Codex UI |
| GitHub Copilot | Coding agent (ephemeral GitHub Actions environment) | assign a GitHub Issue, `@copilot` in a comment, the Agents panel, VS Code | draft PR with commits on the agent branch (the agent does not mark PRs ready-for-review) | Agents panel, IDE, GitHub Mobile push; every commit links back to its session log |
| Cursor | Cloud Agents (renamed from "Background Agents" - treat the old name as legacy) | chat UI, dashboard, or an event trigger (Slack message, new Linear issue, merged GitHub PR, PagerDuty incident) | PR on a new `cursor/<slug>` branch by default; your working branch is untouched | email, desktop notification, Slack if connected - check the default model tier before a long run |
...
The spec-then-loop pattern has ready-made harnesses that build a version of the state-on-disk trio in for you - best known is the Ralph family. [ralph-claude-code](https://github.com/frankbria/ralph-claude-code) (as of mid-2026) wraps repeated agent runs against a PRD with exit detection, rate limiting, and a circuit breaker. It preserves session continuity by default; use its explicit fresh-iteration option when you need each run isolated. A harness handles the loop mechanics; the spec, the walls, and the worst-iteration question stay yours.
