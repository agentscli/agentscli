...
<YouTubeEmbed id="3VbsqaZ5QRs" title="Skills" />
...
**Travels unchanged:** the `name` and `description` frontmatter - required by five of the six tools, and optional in Claude Code, which recommends `description` and defaults `name` from the folder - the Markdown body whatever is in it, and optional bundled `scripts/`, `references/`, and `assets/` alongside `SKILL.md`. Copilot and OpenCode both cap `name` at 64 characters and `description` at 1024; the other tabs don't state a limit, so write to the tighter pair.
