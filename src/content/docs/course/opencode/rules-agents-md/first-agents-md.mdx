...
description: "Two facts about feedmill — UTC timestamps and the rate-limit delay — keep walking out of the agent's memory between sessions. Write them once into a project-root AGENTS.md, or let /init draft it, so the rule travels with the repo instead of with you."
sidebar:
