...
description: "A run edited three feedmill files you didn't want touched, and the thread is creeping toward the context limit. Step the changes back with /undo, walk them forward with /redo, and let OpenCode's automatic compaction summarize a long thread before it overflows - turning it off only when you need the raw history."
sidebar:
...
`/undo` steps back over recent messages, not a whole afternoon of tangled work. For anything you've already proven good, the real floor is a Git commit - `/undo` is the shallow safety net, `git commit` is the deep one. There have also been reliability reports where `/undo` rolled back the *message* without reverting the *file changes* on certain releases - and a worse one where it reverted files to an incorrect old state and deleted committed code. Treat it as your fast first reach, but verify the files actually reverted (`git status`, or re-run "show me everything you changed") before you trust it on a destructive run.
:::

{/* VERIFY (kept - OPEN, recurring class of bug, not version-pinned): /undo file-revert reliability is an ongoing problem class in anomalyco/opencode, not a single fixed bug. Confirmed reports: #5474 (message rolled back but file changes left in place, v1.0.152), #10287 (files reverted to an incorrect old state, ~1300 lines of committed code deleted, Jan 2026). Still-open reports persist into the v1.16 era: #27664 (/undo does not restore code, /redo then commits all changes), #27873 (file watcher fights an intentional undo), #4704 (/undo + /timeline undo don't revert edits). No changelog entry confirms a blanket fix as of v1.16.2, so keep the caution box and the "verify the files actually reverted" guidance regardless of installed version. Re-check the open undo issues before publishing. */}
