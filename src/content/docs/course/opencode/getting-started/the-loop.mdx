...
3. **Approve** - it stops and waits for you. Permissions in OpenCode are per-tool per-agent: the built-in `plan` agent defaults its file edits and bash commands to `ask`, while the built-in `build` agent is the full-tool primary agent. The current [permissions documentation](https://opencode.ai/docs/permissions/) lists `doom_loop` and `external_directory` among the permissions that default to `ask`; most other permissions default to `allow`. Tighten the build policy in config when you want a checkpoint between the agent's intent and your filesystem.
