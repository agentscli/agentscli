...
description: "Ask a model for a video and you get footage you can only re-roll. Route it through code and you get an artifact you can diff, fix one word of, and re-render identically."
---
...
| **You'll have** | A video whose source is a file in your repo - diffable, re-renderable byte-identically, fixable one line at a time |
| **Costs** | Setup is one scaffold command plus a 43s install, then seconds per check. Our 3-second render took 3.1s |
| **Needs** | Node, and a repo. No ffmpeg or Chrome install - both are bundled |
...
<Aside type="tip" title="In a hurry?">
  ...
  npx skills add remotion-dev/skills      # the vendor's own agent skills, as of mid-2026
  ```
...
| Regenerate it next quarter | A different video | The same video, deterministically |
...
<Aside type="note" title="Current picks - verified 2026-07-26, and the fastest-rotting part of this page">
  **Remotion** 4.0.499 is what we ran. 5.0 is announced with a migration guide but not shipped, and it changes licensing terms - re-check before you commit a company to it.
  ...
  **Driving it with an agent:** Remotion **deprecated its own MCP server** in favour of skills - `npx skills add remotion-dev/skills`. Their stated reasons are worth reading, because they generalize: MCP data goes stale against the docs, the vendor pays the token cost, and "installing MCP servers is difficult, and agents do not invoke them reliably." One filed issue has their MCP answering a client's `initialize` eleven seconds after that client had already timed out and given up.
