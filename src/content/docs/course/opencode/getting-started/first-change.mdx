...
Don't reach for the **plan** agent here, even though its name sounds right. Plan is OpenCode's restricted planning agent - file edits and bash are set to `ask`, and its prompt points it at analysis, so it's built to hand you an approach rather than land a change. It's the right tool when you want a plan and nothing written; it's the wrong tool when you want to actually land this fix. You'll switch *to* it later in the course; for your first change you want build with edits gated to `ask`. (You cycle primary agents with the `agent_cycle` keybind, `Tab` by default, but you don't need to switch agents for this - the config does the work.)
...
Treat `/undo` as a convenience for the *last* step, not as your real safety net - that role belongs to Git itself (next section). OpenCode's current TUI documentation says undo relies on Git-backed snapshots and restores file changes when snapshots were captured successfully. Verify the working tree after an undo before treating it as a destructive recovery mechanism.
...
:::tip[Try it]
Open feedmill (or your own Go repo) and describe a small fix in one sentence. When the approval prompt appears, read the diff before you type `y`. If anything looks off, type `n` and rephrase. Commit the moment the diff matches your intent.
:::
