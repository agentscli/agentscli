...
The file format has largely converged. Every tool in scope reads [`AGENTS.md`](https://agents.md/) natively except Claude Code, which reads `CLAUDE.md` and pulls the other in with an `@AGENTS.md` import. So the clean pattern is one `AGENTS.md` at your repo root, imported into `CLAUDE.md` - a single source of truth every major tool will load. The [Comparison](#comparison) table has the per-tool filenames.
...
Tools agree on *where* rules files live but not on *how they combine* - and the differences bite in monorepos. Same repo, same files, a different answer per tool: pick where the agent is working and see exactly which files each one loads. Watch the OpenCode tab: it **replaces** rather than merges.
...
| Nested files per subdir | Concatenated (additive) | Concatenated; deeper files override by appearing later | First match wins (no merging) | Nested `AGENTS.md` auto-applied in subtree | Nearest `AGENTS.md` takes precedence | Nested walk, concatenated; closest file wins |
