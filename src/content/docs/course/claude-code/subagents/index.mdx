---
title: "Subagents"
description: "Some work is worth doing but not worth keeping in your main thread. A subagent does it in an isolated context and hands back only the conclusion."
---
...
Subagents are different from **agent teams**. Teams are an experimental, higher-cost mode in which independent Claude sessions share a task list and message one another directly. Use a subagent when only the result matters; use a team when workers need to coordinate, and enable it only after checking the current [agent teams documentation](https://code.claude.com/docs/en/agent-teams).
