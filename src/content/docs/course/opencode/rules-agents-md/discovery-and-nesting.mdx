...
Here's the mechanism, and it's worth holding precisely because most people arrive expecting the opposite. When OpenCode builds its context for a turn, it doesn't read only the rule file in your current directory. It **walks up** from the current working directory toward the worktree root, and at every level it collects the `AGENTS.md` it finds there. Every one of those files is loaded and **combined** into the rules layer - the nested file does not replace the root, it stacks on top of it. {/* VERIFY: walk-up-upper-boundary */}
...
```text
 ~/.config/opencode/AGENTS.md     global rules        (every project)
        │
        │   combined, not overridden
        ▼
 feedmill/AGENTS.md               repo-root rules     (walk stops here - the git boundary)
        │
        │   combined, not overridden
        ▼
 feedmill/server/AGENTS.md        subdirectory rules  (read last, most specific)

 result: one rules layer = global + root + subdirectory, all in force at once
```
...
If you've come from a tool that only reads the **nearest** rules file - or only the root one - then dropping a file in `server/` *feels* dangerous, because there it would override rather than add. {/* VERIFY: third-party-nesting */}
