...
description: "You keep re-typing the same bank-import procedure into the chat. Build it once as a SKILL.md in .agents/skills/ and Codex follows it without you in the loop."
sidebar:
  label: "Create · SKILL.md"
---

You've decided the six-turn re-narration from the last lesson is worth writing down. Good instinct - the test for "this should be a Skill" is exactly the one you just applied: *you keep re-typing the same multi-step procedure into the chat.* That's the signal. Let's turn it into a directory Codex can follow without you in the loop.
...
That split is deliberate, and it's worth pricing once instead of taking on faith. At the start of a session Codex loads only the frontmatter of every Skill it can see, which for our file - just `name` and `description` - is about 80 tokens at four characters per token. **80 tokens. Paid every session, whether or not you're importing a bank that day.** The body below runs about 280 tokens, and those never enter the window until the Skill is judged relevant - call it 360 tokens for the whole file, with three-quarters of it staying on disk most sessions - tokens, and only tokens, is the instrument this section is really pricing. Turns are next lesson's job, once the Skill actually fires on a real import.
...
- **The cents-not-float step, restated.** This rule already lives in your `AGENTS.md` as a standing fact - but the import path is exactly where a *new* format detail slips past it, so step four repeats the rule at the precise moment a fresh bank's decimal string is in front of Codex - the exact miss you traced in the last lesson. `AGENTS.md` holds the general rule, and the Skill holds the check that catches the bank-specific way it goes wrong. A Skill can carry the judgment a procedure needs, not just the commands. (We'll come back to *when* a fact belongs in the Skill body versus the rules file in the [last lesson](/course/codex/skills/vs/).)

**Written down once, that step four line stops being something you have to remember to say - it's just there, every time the Skill fires, at the exact moment it matters.**
...
:::tip[Try it]
Write a `SKILL.md` for the bank-CSV import procedure (or a repeatable task in your own repo). Start a fresh session and type the situation naturally - "I got a new CSV from the bank." See if the skill fires; if it doesn't, revise the `description` until it does.
:::

The file exists on disk now, priced at about 360 tokens and mostly dormant. What it can't tell you yet is whether Codex actually reaches for it when a new bank shows up unannounced - whether the six turns from the last lesson genuinely become two, or whether the `description` is too vague to earn its keep. That recognition lives entirely in one field. [Let's make it fire at the right time, and only the right time](/course/codex/skills/trigger/).
