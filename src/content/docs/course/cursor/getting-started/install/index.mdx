...
The one-click **VS Code Import** brings over four things - **Extensions, Themes, Settings, and Keybindings** - from a detected VS Code install. You reach it from Cursor Settings (<kbd>Cmd/Ctrl+Shift+J</kbd>) under **General → Account → VS Code Import**, where an Import button does the transfer in a single step. Two inheritances are worth calling out because they keep working without any import step at all:
