...
The first three tiers, as they'd sit in a config:

```text mark={2}
allow:  Read  Grep  Glob  Bash(pnpm test)  Bash(git status)  Bash(prettier *)
ask:    Bash(npm install *)  Bash(git push *)  Edit(package-lock.json)
deny:   Bash(rm -rf *)  Bash(sudo *)  Edit(.env)  Write(outside the project root)
```
...
<YouTubeEmbed id="l_S0WQqtkus" title="Permissions, sandboxing & approval modes" />
