...
Before you write the `/stash` command or the re-extract tool, it's worth seeing the skeleton every Pi extension shares - and worth stating exactly what that skeleton buys you, so you can check the claim instead of taking it on faith. Every extension you build this chapter is a file that gets discovered, gets loaded, and gets wired into the session by calling exactly one of three functions: `registerCommand`, `registerTool`, or `on`. Those are the only three doors in. And each of the ways an extension can go wrong maps to one of those same three stages: put the file in the wrong place, or work in a project Pi doesn't trust, and it's never discovered; make a mistake Pi's loader can't parse, and it's discovered but never loaded; call the wrong one of the three functions - or call it with the wrong arguments - and it loads but doesn't do what you meant. Keep that three-stage map in mind. The next two lessons are just filling in the details at stage three: `/stash` calls `registerCommand`, the re-extract tool calls `registerTool`, and the hook that ties them together calls `on`.
...
One more thing worth knowing before you trust either your own code or Pi's: `ExtensionAPI` is not a frozen contract. Pi is young and the surface has already moved once - the npm scope rename above is one visible sign of it - and a method's shape can change between releases the same way any actively developed API's can. If an extension that worked last month throws after you upgrade Pi, check what changed before you assume your own code is what broke.
...
While you're iterating, you don't have to touch either folder at all - `pi -e ./stash-tools.ts` loads a specific file for just that run, which is the fastest way to try an idea before you've decided it's worth keeping around permanently. It also doubles as your test bed: point `-e` at a half-finished tool and check what it actually does against a throwaway case before you ever let it run loose in a session that touches real data.
