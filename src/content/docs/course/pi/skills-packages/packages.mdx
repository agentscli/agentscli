...
description: "Extensions, Skills, and prompt templates so far have lived in one repo's .pi/ directory. A Pi package bundles them and ships over npm or git — pi install npm:@you/stash-harness and a teammate has the whole thing, not a README to skim."
sidebar:
  label: "Packages"
---
...
Pi has no bespoke registry, so the gallery is really just "npm packages someone tagged `pi-package`" — reportedly several thousand of them at last count, filterable by resource type and sortable by downloads. A few worth knowing about for landscape, not as an endorsement to install any of them blindly: a web-access bundle that adds search/fetch/GitHub-clone tools, a sub-agent task-delegation package, an MCP bridge (the community route into MCP support you saw referenced back when you [built your first extension](/course/pi/first-extension/) — still not core Pi, still a package like any other), and a cross-tool context/MCP plugin that also targets Claude Code and Gemini CLI. The shape of that list is itself informative: on Pi, "add MCP support" and "add sub-agent delegation" are both just `pi install` away, not built-in features you're missing.
