...
Read that the way OpenCode reads it: deny everything, then carve back the audit family. The `add-feed` skill matches `*`, hits `deny`, and never appears as something the auditor can load - while an `audit-parser-coverage` skill stays available. Write the catch-all *first* and the specific exceptions after it - the same idiom you used for `bash` command maps in the [permissions chapter](/course/opencode/permissions/per-tool-per-agent/), and for the same reason: it reads top-down the way you'd reason about it, and it lands the outcome you want without you having to know which internal rule breaks the tie. {/* VERIFY: do NOT re-assert "last-match-wins" here. The mechanism is unsettled between opencode's docs (last matching rule wins) and its v1.16.2 source (patterns sorted by length, most specific wins) - see the full note at permissions/per-tool-per-agent.mdx. Catch-all-first is the idiom both rules agree on; keep the prose on the outcome, not the mechanism. */} Flip the polarity when the agent is broadly trusted and you only want to fence off a few procedures:
