...
Back in the first chapter you met the idea that Copilot is one assistant wearing several surfaces - completions, chat, the modes. Here's the surface that proves it. Everything you've done so far happened inside VS Code, with you watching. The **cloud coding agent** is a related Copilot surface, reading supported repository instructions and skills - but it doesn't run in your editor. It runs on GitHub, asynchronously, while you do something else, and it hands its work back as a pull request. Surface support and organization policy still matter: a local IDE setting or local MCP server does not automatically carry over.
