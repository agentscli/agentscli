...
> what does this service do, and where does it parse dates out of imported CSVs?
  ...
  app/importers/csv.py; that's where each row's date string is parsed
  into a date before the transaction is stored. The parsing assumes a
  single fixed date format, which won't hold across different banks.
```
...
3. **Approve** - it stops and waits for you. By default, nothing touches your disk and no command runs until you've said yes. This is the checkpoint between the agent's intent and your filesystem, and it's there on every change.
