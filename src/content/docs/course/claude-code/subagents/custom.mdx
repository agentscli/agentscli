...
Same isolated context, same terse return - but now reproducible. The worker that took a paragraph of briefing every time is a single name, and everyone who pulls the repo runs the identical audit. (One boundary worth knowing, from the docs: subagents *can* [spawn nested subagents](https://code.claude.com/docs/en/sub-agents#spawn-nested-subagents) as of v2.1.172 - nesting is allowed down to a fixed depth of 5, and only the top-level summary returns to the main thread. To stop a subagent from spawning others, omit the `Agent` tool from its `tools`.)
