...
That second question is the whole trust model of unattended work in one line. The agent ran with nobody watching, so the checks in its environment decided what "done" meant — and a green board can mean less than it appears to. Run a night's batch and watch what each check actually buys:
