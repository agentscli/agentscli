...
description: "You've narrated 'how we onboard a new feed format' three times this week. Write it once as a SKILL.md with the required name and description frontmatter under .opencode/skills/, and let OpenCode's skill tool load and run it on demand the next time a feed shows up."
sidebar:
...
Read that first beat. You never said "use the onboard-feed skill" - OpenCode chose it from the description and pulled the body in with the `skill` tool, the same way it'd reach for `read` or `bash`. From there it ran *your* four steps, in *your* order, with the fixture test you'd otherwise have to remember to ask for. The procedure that lived in your head, and that you'd narrated three times, now lives in the repo and runs on recognition. {/* VERIFY: TUI rendering of the skill load - the native `skill` tool is confirmed (opencode.ai/docs/skills: skills load "via the native skill tool", invoked as skill({ name: ... }), and the load returns a confirmation message naming the skill). The exact TUI line format used in the transcript above ("Skill onboard-feed / loaded: ...") is illustrative and not pinned by the docs; treat the rendered shape as approximate until checked against a live v1.16.2 session. */}
...
One procedure, written once, now owned by the tool. The next time you add a feed you describe the source and read the diff - the four-step dance is OpenCode's job. But you wrote this skill for OpenCode, in OpenCode's path. The next lesson takes the more interesting claim at face value: that the *same folder* works when you're driving Claude Code on this repo, with no rewrite - and looks at how far that portability stretches to other tools that read the same skill paths. {/* VERIFY: cross-tool portability - CONFIRMED for Claude Code: opencode.ai/docs/skills lists .claude/skills/ and .agents/skills/ (project + global) among its six skill search paths and states "Unknown frontmatter fields are ignored", so a Claude Code skill folder is read unchanged. NOT documented: Codex skill compatibility - opencode's docs do not mention Codex anywhere, so any "loads unchanged in Codex" claim is unverified; do not assert it. Keep the cross-tool framing scoped to Claude Code / .agents until Codex support is confirmed. */} [Next: reuse skills across tools](/course/opencode/skills/portable-skills/).
