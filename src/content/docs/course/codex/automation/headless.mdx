...
description: "Run the agent without the chat. codex exec turns Codex into a plain command you can pipe into, script around, and read a result back from."
sidebar:
...
When a *script* needs to act on the result rather than just store it - pull out a single number, branch on whether anything overspent - reach for structured output instead of parsing prose. `codex exec` can emit a [machine-readable event stream and constrain its final answer to a schema](https://learn.chatgpt.com/docs/developer-commands?surface=cli); that turns the agent into a typed step in a pipeline rather than a thing you read by eye. For the month-end report, plain stdout redirected to a file is all you need; we'll lean on structured output when CI has to *decide* something in the [CI lesson](/course/codex/automation/ci/).
