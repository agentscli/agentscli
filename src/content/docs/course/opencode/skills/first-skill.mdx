...
description: "You've narrated the same procedure three times this week. Write it once as a SKILL.md under .opencode/skills/ and let the model load it on demand."
sidebar:
...
You've now walked OpenCode through onboarding a new feed source three times this week - sniff the format, write the adapter, register it, add the fixture test - and all three times you typed the same four-step narration with a different feed plugged in. That re-narrating is the whole waste. The steps are stable; only the feed changes. So you write the procedure down once, in a form the model can load when it is relevant, and you stop being the thing that remembers the routine.

That form is a **Skill**: a directory with a `SKILL.md` in it. The frontmatter tells the model *when the skill may apply*; the body tells it *what to do*. OpenCode keeps every skill's name and description in view and exposes them through a built-in `skill` tool - when the model decides your request matches one, it may call that tool to pull the body in and follow it. You never need to paste the procedure again, but review the resulting tool calls as usual.
...
Here's the payoff. A new feed shows up - a podcast that publishes a JSON feed - and you say the thing you'd say to a teammate, with no mention of the procedure. The model can match your request against the skill descriptions it is holding, choose `onboard-feed`, and call the `skill` tool to load the body:
...
One procedure, written once, now owned by the tool. The next time you add a feed you describe the source and read the diff - the four-step dance is OpenCode's job. The next lesson examines how far the shared Agent Skills format travels across tools that read the same skill paths; tool-specific gating and frontmatter still need separate verification. [Next: reuse skills across tools](/course/opencode/skills/portable-skills/).
