...
For the per-tool mechanics, see [Slash commands](/foundations/slash-commands/) for absorbing the repro workflow into a repeatable command, [Headless & CI](/foundations/headless/) for replaying scenarios on every change, and [Subagents](/foundations/subagents/) for isolating scenario authoring from the fix.
