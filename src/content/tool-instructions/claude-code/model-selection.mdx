...
**Switching:** `/model` mid-session lists available models; choosing a row with Enter (or `/model <alias>`) saves it as your default for new sessions - press `s` in the picker to switch for the current session only. Pin a default via the `model` setting or the `ANTHROPIC_MODEL` env var.
