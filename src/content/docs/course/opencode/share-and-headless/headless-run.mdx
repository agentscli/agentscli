...
:::tip[Try it]
Run `opencode run -p "list all exported functions" --format json` in feedmill. Pipe the output through `jq` and extract just the text. Set a permission to `ask` and run it again - notice how the run stalls waiting for input.
:::
