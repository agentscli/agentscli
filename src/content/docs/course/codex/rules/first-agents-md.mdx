...
Notice what's *not* in there. Nothing says "write clean code" or "follow Python best practices" - Codex already aims for those, and the words would just burn context. Nothing transcribes the directory layout or the framework name; Codex reads those from the repo. Every line is a fact that's specific to `budgetcli`, true beyond this week, and something the code itself either omits or gets wrong. This is the bar every line has to clear, and the [good-rules lesson](/course/codex/rules/good-rules/) sharpens it further.

## What it actually costs

Now count what you just wrote, because the whole chapter turns on this number. Thirty-two lines, 1,137 characters including the blank ones - and by this course's prose estimator, four characters per token, that's roughly **284 tokens**. Not 284 tokens once. **284 tokens every single session**, from the moment you save this file until the day you delete it, whether that session ever touches an amount field or not. That's the rent named in the last lesson, priced for real for the first time. Hold that number - the next two lessons spend it against what the float-money bug actually costs when nobody's written this down.
...
Three facts you used to repeat every morning, written down once, now get paid for once per session instead of once per mistake - the whole trade in one line.

:::tip[Try it]
Write three rules for `budgetcli` in `AGENTS.md`: one about money handling (always integer cents), one about test commands, one about import style. Start a fresh session and ask Codex to add a new endpoint. Check whether it followed your rules without being reminded.
:::

One file at the repo root covers the team-shared truth about `budgetcli`, and its 284 tokens are the same whether the file is this lean or twice as long. That's exactly why the next question isn't "what else should I write" but "where else should it live" - some facts are personal, some only matter in one subdirectory, and paying rent on them everywhere is money spent on a session that will never use them. [Next: how Codex discovers and layers several `AGENTS.md` files at once.](/course/codex/rules/the-hierarchy/)
