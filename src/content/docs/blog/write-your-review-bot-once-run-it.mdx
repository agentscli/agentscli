...
For a committed branch, fetch the base and pipe `git diff "origin/$BASE...HEAD"` so the review covers changes since the merge base; the two-dot/working-tree form above is only appropriate when that is deliberately the state you want to review. The session's only capability is now reading repo files for context, and the diff arrives as stdin rather than as a command the diff itself can influence. The ledger does not change: the diff still costs 1,440 tokens, it just rides in through the prompt instead of a tool call.
