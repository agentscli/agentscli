...
| Cursor | VS Code's `settings.json` model; `.vscode/settings.json` is honoured; exact scope precedence follows the VS Code/Cursor settings model | The CLI accepts only *permissions* per project; every other CLI setting is global |
...
For Codex, the [current config reference](https://developers.openai.com/codex/config-reference/) is the source of truth: project config is conditional on trust, and profile files are versioned configuration details rather than a portable convention.
