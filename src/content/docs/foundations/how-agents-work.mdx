...
You asked for a one-line change and the agent read four files you never mentioned, ran the test suite twice, and came back three minutes and several thousand tokens later. Or the opposite: an hour into a session that started sharp, it quietly drops a constraint you set forty minutes ago, and you find yourself re-explaining something it clearly understood at the time.

Neither is random, and neither is fixed by prompting harder. Both are the shape of the loop the agent runs, and the size of the buffer it runs inside. Without a model of those two things, every surprise reads as the model being clever or being dumb, and the lever that would actually change the outcome stays invisible.

That lever is what the rest of this section is about, one primitive at a time: **how do you control what the model sees, in what order, and at what cost?** This page is the substrate underneath all of them.

<YouTubeEmbed id="UcYf9slLQoc" title="How agents work" />
...
<FlowDiagram
  steps={[
    { point: 'you' },
    {
      group: '',
      direction: 'row',
      loop: 'repeat until done (you can interrupt at any point)',
      steps: [
        { node: 'gather context', sub: 'read, grep' },
        { node: 'act', sub: 'edit, run' },
        { node: 'verify', sub: 'test, re-read' },
      ],
    },
  ]}
/>
...
The diagram above, running. A one-line bug fix traced step by step - including the verify that fails and sends the loop back around, which is the part a diagram can't show.

<LoopStepper client:visible />
<noscript><p class="widget-noscript">This loop stepper needs JavaScript. The surrounding explanation remains available without it.</p></noscript>
...
<StackDiagram
  panels={[
    { title: 'context window (finite)', rows: [
      { label: 'system prompt', note: "the CLI's own instructions" },
      { label: 'rules', note: 'AGENTS.md / CLAUDE.md, loaded at start' },
      { label: 'loaded skills', note: 'descriptions always, bodies on demand' },
      { label: 'tool definitions', note: 'what the model can call' },
      { label: 'conversation', note: 'your messages + model replies' },
      { label: 'tool call results', note: 'file contents, command output, search' },
    ]},
  ]}
/>
...
| Profile | Cost | Examples |
|---|---|---|
| **Always-loaded** - read into the window at session start, present on every turn | Permanent footprint until the session ends | [Rules](/foundations/rules/), tool definitions, [skill](/foundations/skills/) descriptions |
| **On-demand** - their *existence* is visible to the model, but the *content* only loads when the model decides it's relevant | Cheap until used | [Skill](/foundations/skills/) bodies, MCP tool schemas (where the CLI supports lazy loading) |
| **Isolated** - spawned with their own fresh window, do work, return a distilled result | No automatic transcript on the parent window; some clients retain an inspectable worker thread | [Subagents](/foundations/subagents/), [plan-mode](/foundations/plan-mode/) explorations you discard |
