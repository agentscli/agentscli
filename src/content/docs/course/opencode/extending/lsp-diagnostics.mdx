...
description: "An edit that compiles in the agent's head can still break types you never see in a diff. OpenCode spawns LSP servers by extension and feeds it live diagnostics."
sidebar:
...
OpenCode does not replace the language server or the Go toolchain. Install `gopls` and make sure it is on your `PATH`, then verify it with `which gopls`; if either `go` or `gopls` is missing, install the Go tooling before debugging the OpenCode configuration. The one thing this setting controls is whether OpenCode starts the configured server and feeds its diagnostics into the loop.
