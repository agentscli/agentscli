...
<Aside type="tip" title="In a hurry?">
  ...
  ```bash
  $ grep -nE '#[0-9a-f]{6}|[0-9]+px' src/components/X.tsx   # zero hits = wired right
  ```
</Aside>
...
| Codex | Attach in the ChatGPT and IDE surfaces alike | MCP configured like any other server |
...
| Pi | Whatever the harness supports | None - no native [MCP](/foundations/mcp-servers/), so every ledger row has to be carried by the anchor text |

## Off the shelf

If your designs live in Figma, some of the plumbing already ships. Figma's official [skills for MCP](https://help.figma.com/hc/en-us/articles/39166810751895-Figma-skills-for-MCP) (verified mid-2026) come bundled with the official Figma plugin: `figma-generate-design` builds full screens from your design system, `figma-code-connect` wires components to their code counterparts.
