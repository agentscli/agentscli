...
description: "Connecting an MCP server is one config file, .vscode/mcp.json. We point Copilot at the internal registry that knows who imports shared-lib."
sidebar:
...
:::tip[Try it]
Add a simple MCP server to `.vscode/mcp.json` - a filesystem or search server works well for a first try. Ask Copilot a question that requires that server's tools to answer. Confirm the trust prompt appears before the server starts; if it connects silently, something in your config skipped the gate.
:::
