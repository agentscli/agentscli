...
description: "One of feedmill's feeds carries content your employer won't let leave the network, so a hosted frontier model is off the table for that task. Point OpenCode at a local Ollama or LM Studio model - or any OpenAI-compatible endpoint via a custom provider block - and let Zen pick for you when 75 providers is too many to choose from."
sidebar:
