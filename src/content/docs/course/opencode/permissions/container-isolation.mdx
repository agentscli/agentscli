...
OpenCode's [permissions documentation](https://opencode.ai/docs/permissions/) describes a **tool-level policy**, not an OS-level sandbox. Its rules gate tools such as `edit`, `bash`, and `read`. When you set `bash: "ask"`, you're saying "stop and ask before invoking the bash tool"; you are not configuring the operating system to deny the resulting process access to your home directory, SSH keys, or network. For OS-level containment, put the whole process in a container or another sandbox you control.

This matters more than it sounds, because a permission check is not process containment. `external_directory` gates paths supplied to tools, including many file operations and bash calls, but it does not make an approved shell command harmless. Treat the per-tool map as an approval boundary and use a container when the job needs real isolation.
