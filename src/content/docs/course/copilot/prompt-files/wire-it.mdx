...
description: "The frontmatter that turns a one-shot prompt into a flexible command: an argument hint, the mode, the model, the tools it may use - plus one precedence rule."
sidebar:
...
If an agent seems to have access it shouldn't - or lacks access it should - check whether a prompt file is overriding its `tools` list. The prompt always wins, silently, for that invocation.
:::
