...
Read that command carefully, because the ordering bites people. Transport and environment options come before the server name in this form; scope placement follows the current CLI syntax, so copy the [MCP command reference](https://code.claude.com/docs/en/mcp) rather than assuming every option has the same position. The bare `--` then separates the name from the command Claude Code will actually spawn to run the server. (Note the `readonly` user in that connection string - we'll come back to why that choice matters in the next lesson.)
...
(Check the vendor's own MCP docs for the exact endpoint - they publish it; don't guess.) Cloud servers often need authentication. HTTP MCP can use OAuth, bearer/header authentication, or a server-specific helper. Follow the server's documentation and use secure storage or environment expansion; never paste a password or long-lived secret into a checked-in config file.
...
One discipline before you connect a third server: mounted servers add context overhead, although tool search usually defers full definitions until needed. Providers and models can differ, so monitor the actual cost with `/context` rather than assuming every server has the same startup footprint. Mount and unmount below and watch what each one costs:
