---
title: MCP servers
description: How each CLI consumes Model Context Protocol servers - and why Claude Code and Codex can also be one.
---
...
<YouTubeEmbed id="-p1gdYroWxg" title="MCP servers" />
...
- **Which side you're on.** Most tools in scope are MCP *clients* only. Two can also *be* a server over stdio, and they expose opposite things. `codex mcp-server` offers two tools, `codex` and `codex-reply`: you hand it a task and it runs a whole Codex turn. `claude mcp serve` offers Claude Code's own primitives instead - `Bash`, `Read`, `Edit`, `Write`, `WebSearch`, `Agent` and the rest - so the caller keeps the reasoning and borrows the toolbox. Delegate a turn, or borrow the tools: decide which you want before wiring either up. And don't read a `serve` subcommand as MCP by default, because `opencode serve` starts OpenCode's own HTTP API, not an MCP endpoint.
...
| Can be an MCP server | **Yes** (`claude mcp serve`) | **Yes** (`codex mcp-server`) | No | No | No | No |
...
- Claude Code and Codex both hold a dual MCP role - *consumer* and *producer* - so "an MCP server" in either conversation can mean something the tool calls or the tool itself. Their own subcommands make the ambiguity concrete: `codex mcp` manages the servers Codex calls, while `codex mcp-server` turns Codex into one. Same word, opposite directions, one letter apart.
...
<UpstreamRefs checkedAt="2026-08-13"
  links={[
    {
      tool: 'Claude Code',
      label: 'MCP',
