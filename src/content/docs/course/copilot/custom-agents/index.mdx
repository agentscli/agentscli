...
description: "Act 1 opens. The shared-lib change touches twelve consumers, so it needs care you don't want to retype every session. Custom agents turn a reusable persona - a reviewer that already knows the library's rules - into a file, and subagents let one agent fan delegated work out across all twelve consumers in isolated context."
---
