...
<YouTubeEmbed id="gdiB-2pSE5Y" title="Plan mode / read-only agent" />
...
<noscript><p class="widget-noscript">This plan-mode stepper needs JavaScript. The <a href="#comparison">comparison table</a> below remains available without it.</p></noscript>
...
**Blocked by the runtime.** Claude Code's mode blocks Write and Edit for as long as the toggle is on, so a wrong turn fails closed and costs you nothing but the tokens. Bash is gated, not blocked - commands are prompted or classifier-reviewed, and approved commands run. One carve-out matters: in a session where bypass permissions are available, plan mode's blocks are *not* enforced - Claude is still instructed to plan rather than edit, but an edit it attempts during planning will run.

Codex composes the same posture out of permission primitives, and its version is the sturdier one: under `--sandbox read-only` the restriction is enforced below the agent, so it doesn't depend on the session's other flags.

**Gated, not blocked.** OpenCode's `plan` primary agent is the clearest example of this class on the current V1 docs: edits map to **ask** (bash ask too), so a proposed edit surfaces for your approval instead of executing. OpenCode's V2 agent docs harden that further - edits are denied except for OpenCode plan files - so check which docs line your install follows. Either way, the posture is only as read-only as the permission map, which is a different guarantee from a runtime block.
...
Cursor documents Plan Mode as a research-and-plan workflow, while its public docs do not publish an exhaustive tool allowlist. Treat it as workflow guidance, not as a security boundary. *Unverified:* its exact runtime blocking behavior depends on the Cursor surface and version. See the [Plan Mode announcement](https://cursor.com/blog/plan-mode) and [agent modes documentation](https://cursor.com/docs/agent/plan-mode).
...
| Plan artefact | Inline chat output | Inline chat output | Inline chat output | **Editable markdown file** (location is version- and workspace-dependent) | Structured plan in chat (session-scoped) | A file you ask the agent to write, by convention |
...
**The plan lives in a file.** Cursor's mode can produce an editable markdown plan rather than only chat. The location and save workflow are version- and workspace-dependent, so confirm them before assuming the plan is outside the repo or absent from the diff.
