...
description: "You have parallel workers each on their own branch. Orchestration is the thin layer that ties them together: a read-only reviewer gating each hand-off, an aggregation step you wrote before you fanned out, and a sequential gated merge that never lets a silent failure slip into main."
sidebar:
