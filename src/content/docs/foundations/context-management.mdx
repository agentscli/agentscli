...
The vocabulary differs; the mechanism has standardised. Every tool in scope, when the window approaches full, makes a single summarisation call and replaces the bulk of the history with a **structured Markdown summary**. They reached that design independently, and the structure is nearly identical across them:
...
Several tools expose a manual `/compact` that takes optional instructions (`/compact focus on the API changes`) so you can bias what the summary preserves. Understand compaction in one tool and you understand it in all of them. What still differs is *how much control you get over when it fires and what it keeps* - that is the [comparison](#comparison) below.
...
| Custom compact instructions | Yes (inline + `CLAUDE.md`) | Yes - `compact_prompt` (inline string) or `experimental_compact_prompt_file` (file path, experimental) <sup>1</sup> | Fixed structured template | Not exposed *Unverified* | Yes (inline) | Via extension |
...
<sup>1</sup> Codex's custom prompt only governs its *local* compaction path. Providers that support remote compaction skip the local path entirely, and `compact_prompt` isn't read on that path either, per the Codex source (as of 2026-07).
