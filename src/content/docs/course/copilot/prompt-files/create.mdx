...
:::tip[Try it]
Create a `.prompt.md` file for a task you repeat often - "write tests for the selected file" is a good first one. Invoke it with `/` in the chat, then type the same instruction freehand in a new conversation and compare the two results. The prompt file should give you the same shape every time; the freehand version probably won't.
:::
