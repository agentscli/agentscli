...
The four modes are a ladder of latitude - each rung hands the agent more of the work, and widens its blast radius to match:

```text
 Ask ──────▶ Edit ──────▶ Agent ──────▶ Plan

 1  Ask     read-only, changes no files              blast radius: none
 2  Edit    scoped writes to the files you pick       blast radius: one area
 3  Agent   multi-file edits + terminal, self-corrects  blast radius: whole repo
 4  Plan    researches the approach, then executes    blast radius: repo, plan reviewed first
```
