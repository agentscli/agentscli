...
That's the next chapter. **[Extending Codex](/course/codex/extending/)** gives the agent reach into external systems with MCP servers - that exchange-rate API your transactions have been waiting on - and deterministic checks that can run at supported lifecycle points with hooks. For security-critical conditions such as "never write to the real ledger," combine those checks with database permissions, sandboxing, credentials, and CI rather than relying on a hook or prompt alone. Skills taught Codex your procedures; next you give it new things to do them *to*, and new ways to draw a hard line.
