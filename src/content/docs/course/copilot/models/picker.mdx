...
description: "A model picker sets the reasoning behind each request. The judgment is knowing when a gnarly shared-lib refactor wants more thinking and routine CRUD less."
sidebar:
...
The picker is deliberately not a fixed catalog. The available models change as GitHub adds and retires models, and your plan, organization policy, surface, and region can affect what appears. Use the model picker and GitHub's current supported-models documentation as the source of truth rather than memorizing model names.
...
:::tip[Try it]
Open the model picker and note what's available on your plan. Give Copilot a mechanical task - rename a variable across a file - on the default model, then switch to the strongest model available and ask a design question about the same code. Compare the depth of the two answers; the rename shouldn't get better, and the design question probably should.
:::
