...
description: "Don't write a CLAUDE.md from a blank page. Run /init and Claude reads your codebase, then drafts a baseline of the commands and conventions it can discover."
sidebar:
...
A few things worth knowing about how `/init` behaves, because they shape how you use it. If a `CLAUDE.md` already exists, `/init` doesn't clobber it - it reads what's there and *suggests improvements* instead. With the newer initialization flow enabled, `AGENTS.md` can also be considered; do not assume every version reads it by default. If your repo carries `.cursorrules`, verify how the installed version handles it rather than assuming every instruction is imported. The exact behavior, including the optional interactive multi-phase flow, is documented on the [memory page](https://code.claude.com/docs/en/memory).
...
:::tip[Try it]
Run `/init` in your own repo. Read the generated CLAUDE.md and delete every line that states the obvious. Keep only rules you'd tell a new teammate on day one. Commit the trimmed result.
:::
