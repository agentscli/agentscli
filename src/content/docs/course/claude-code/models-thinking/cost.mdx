...
Exact per-token prices move, so the number worth internalizing is the *ratio* between tiers, not the dollar figure. Taking the light tier as 1x, and reading the ratio off the current generation of each tier:

| Tier | Relative cost | Best for |
|------|--------------|----------|
| Light / Haiku | 1x | Renames, formatting, simple lookups |
| Capable / Sonnet | 3x | Multi-file features, code review |
| Strong / Opus | 5x | Architecture, concurrency bugs, planning |

A rename on the strong tier costs five times what it costs on the light tier and finishes no better - that's the whole diagonal in one row. Note the spread is narrower than people assume, which cuts both ways: the strong tier is cheaper to reach for than its reputation suggests, but the light tier is still a 5x saving on work that never needed the reasoning. Both the prices and the models sitting in each tier change; check the [official pricing page](https://www.anthropic.com/pricing) before you budget anything real.
