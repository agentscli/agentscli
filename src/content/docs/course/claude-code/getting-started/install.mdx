...
**`claude: command not found` after install.** The binary landed somewhere your shell doesn't look. On macOS with Homebrew or the native installer, the usual culprit is a missing `PATH` entry - open a *new* terminal window (not a new tab) and try again. If it still fails, run `claude doctor` when available and check the installer's documented location (the native installer commonly uses `~/.local/bin/claude`) rather than assuming `~/.claude/bin/`.

**Node.js version.** The npm install path (`npm install -g @anthropic-ai/claude-code`) currently requires Node.js 22+. Check with `node --version`; if you're below 22, upgrade Node first or use the native installer instead. Confirm the current requirement in the [installation guide](https://code.claude.com/docs/en/installation).
