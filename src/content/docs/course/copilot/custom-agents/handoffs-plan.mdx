...
description: "In supported IDE flows, handoffs can chain agents into one deliberate sequence: plan the shared-lib change, review it against the rules, and audit consumers before code."
sidebar:
...
In frontmatter, the chain looks like:
...
In a supported IDE flow, the plan agent can declare those handoffs; when it finishes producing the approach, the workflow transitions to `library-reviewer`, and from there to `consumer-audit`. On GitHub.com cloud agent, perform these stages explicitly instead.
