...
description: "A million-token window won't fix a long build. Externalize state to a plan file and relay across fresh sessions - quality dies long before the window fills."
authors: sanjay
date: 2026-05-29
...
You can watch that failure happen. Let the session below fill to 75%, then pick **keep going** - that's auto-compact arriving mid-task, with nobody steering what survives. Then replay the `/compact` branch: steering genuinely helps, and for a single-task session it's enough. But hold onto how much still evaporates in the good branch - that residue is why a multi-hour build needs the relay, not a better summary.
...
For the per-tool mechanics, see [Configuration & profiles](/foundations/configuration/) for disabling auto-compact in each tool, [Plan mode / read-only agent](/foundations/plan-mode/) for producing the phased plan without touching code, and [Rules](/foundations/rules/) for loading your conventions into every fresh session.
