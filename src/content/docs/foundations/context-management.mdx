...
|---|---|---|---|---|---|---|
| Inspect usage | `/context`, `/usage`, status line | `/status` | Status display is surface-dependent | input-box gauge | chat input context control | Not listed in this comparison |
...
| Custom compact instructions | Yes (inline + `CLAUDE.md`) | Yes - `compact_prompt` (inline string) or `experimental_compact_prompt_file` (file path, experimental) <sup>1</sup> | Fixed structured template | Not listed in the public comparison docs | Yes (inline) | Via extension |
| Disable auto-compaction | Check current Claude Code settings | n/a | `compaction.auto=false` / `OPENCODE_DISABLE_AUTOCOMPACT` | Not listed in the public comparison docs | `github.copilot.chat.summarizeAgentConversationHistory.enabled=false` | Via extension |
...
| Fork / branch a session | `--fork-session`, `/branch` (verify current command) | `/fork`, `/side` | `opencode --fork` | `/fork` (CLI) | - | **Yes** - native tree-of-sessions (`/tree`) |
