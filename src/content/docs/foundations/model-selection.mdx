...
|---|---|---|---|---|---|---|
| Vendor lock-in | Anthropic only | OpenAI only | **Multi-provider** | Multi-provider (built-in roster) | Multi-provider (curated, plan-gated) | **Multi-provider** (15+ providers) |
...
| Different model per subagent | Yes (frontmatter `model:`) | Yes (TOML `model` field) | **Yes** | Yes | — | N/A (no built-in subagents) |
