...
description: "Pi runs YOLO by default — no sandbox, no approval prompts, full filesystem and shell access on every turn. Build the one gate stash actually needs: a rule file, a tool_call hook, and a hard-stop-vs-let-adapt choice for every violation."
sidebar:
