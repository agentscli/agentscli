...
That function grades uncommitted work against `HEAD`. For a committed pull-request branch, use the merge-base form instead: fetch the base branch, then grade `git diff origin/main...HEAD` (replace `main` with the actual base). Two dots compare endpoints; three dots compare the branch tip with its merge base. Do not call a shallow checkout complete until the runner has fetched enough history to resolve that base.
