...
description: "Four beats, run dozens of times a day: read what's there, propose with the smallest gesture that fits, review the diff, commit the moment it's green."
---

## The loop: read → propose → review → commit

Every gesture in this chapter - Tab completion, the Cmd/Ctrl+K inline edit, the `@` and `/` grammar of a prompt, the review - is a move inside one repeating rhythm. Worn in, it's four beats:
...
One turn, written at the size it actually runs, on `budgetcli` - the course's illustrative TypeScript personal-finance API. The bug for this turn: negative balances render without a sign in the overspend report.

```text
# Illustrative: one turn of the loop, compressed to its beats.
...
review  →  diff: formatMoney change is right - keep. test case asserts
           "-$40.00" - correct, keep. npx vitest run → ok.

commit  →  git commit -m "fix: keep minus sign on negative balances"
```

That's the day. Not one heroic prompt, but the same four beats run dozens of times, each one cheap because you reached for the right-sized gesture and never let an edit land unread.

## Read before you edit

The loop's *read* beat has its own gesture - when you can't yet name the change, drop the agent into a read-only posture before you escalate to one that can edit - but that posture choice belongs to the [modes chapter](/course/cursor/modes/), which owns the Ask → Agent escalation and the <kbd>Shift</kbd>+<kbd>Tab</kbd> chord that cycles postures.
...
- **`@` attaches, `/` runs** - the grammar under every prompt: `@` pins what the agent looks at, `/` fires a command or skill.
...
Every beat of that loop assumes something you haven't yet set on purpose: how far the agent is allowed to go before you look. That posture is one chord away, and it's the next chapter. [Next: the modes](/course/cursor/modes/).
