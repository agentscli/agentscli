...
It's worth being precise about what rewind actually covers, because getting this wrong is how people lose work. **Rewind is the agent's undo, not git.** It rolls back eligible Claude Code file changes during this session. It does not reliably restore Bash-created changes, background or subagent edits, external/manual edits, symlink or hard-link changes, anything you'd already committed, or side effects that escaped the session - a migration that already ran against your database, a branch you pushed. Those live outside the checkpoint and stay exactly as they are.
