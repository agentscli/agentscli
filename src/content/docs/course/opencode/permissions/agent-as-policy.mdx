...
Permission values are `allow`, `ask`, or `deny`, and they're not limited to `edit` and `bash` - the documented keys include `read`, `edit`, `glob`, `grep`, `list`, `task`, `skill`, `lsp`, `question`, `webfetch`, `websearch`, `external_directory`, and `doom_loop`. `list` gates the file-listing tool separately from `glob`. Per-agent permissions merge with your global config, with the agent's rules taking precedence.
