...
> /context
  ...
  Context window: 142k / 200k tokens (71%)   # illustrative example
...
The first lesson hiding in that breakdown is that **MCP servers can cost tokens at rest.** Tool names and server metadata are present, while full tool definitions are usually deferred by tool search when supported. Providers, gateways, and unsupported models can load definitions up front. `/context` is how you catch the tax; the fix is to connect only what the task in front of you needs, and disconnect the rest.

The second is that **a single careless read can dominate.** Ask the agent to "read the whole config module" and a four-thousand-line file lands in context entire. If `/context` shows the conversation lurching upward right after one read, that's usually the culprit - and the habit that prevents it is pointing the agent at the specific function with [`@file`](/course/claude-code/daily-workflow/inline-input/) rather than turning it loose on the whole thing.
...
You can rehearse that reading without burning a real week. Step through this simulated session and watch the window fill - the overhead that's there before you type, the spike when a whole file lands, what survives when the thread gets compacted:
