...
Both loading paths - the once-per-turn walk-up and the read-triggered reminder - are answers to one question: *which files reach the agent, from where.* That question is bigger than rules. Skills, commands, agents, and `opencode.json` itself all draw the same project-versus-home line, and it's worth holding the whole layout once rather than rediscovering it a surface at a time:
