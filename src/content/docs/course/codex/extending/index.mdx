...
description: "budgetcli needs to reach an external exchange-rate API, and it needs a guardrail that holds even when nobody's watching. This chapter adds two extension points: MCP for reach, hooks for a deterministic gate."
---
...
- [Connect the exchange-rate API as an MCP server](/course/codex/extending/mcp-connect/) — a `[mcp_servers.<name>]` block, `codex mcp` to manage it, and the network-off gotcha that bites everyone the first time.
- [Let Codex itself act as an MCP server](/course/codex/extending/mcp-serve/) — the other direction, and when handing Codex to another agent is the right move.
