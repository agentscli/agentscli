...
description: "One engineer, one day, two postures: a careful shared-lib morning on a short leash, a fast orders-service afternoon on a long one. Watch each choice get made."
sidebar:
...
### The reviewer agent and its subagents
...
### Default permissions, higher reasoning
...
### Agent mode, loosened permissions
...
### The cloud agent takes the backlog
...
:::tip[Try it]
Plan your next work session around the mode ladder. Start in Ask mode to understand a problem you didn't write, move to Edit for a contained fix, and only reach for Agent if the change spans multiple files. Note which mode you'd have defaulted to before this course - that reflex is exactly what the last twelve lessons were retraining.
:::
