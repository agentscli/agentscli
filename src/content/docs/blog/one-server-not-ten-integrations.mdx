...
For the per-tool mechanics, see [MCP servers](/foundations/mcp-servers/) for the protocol and server setup, [Permissions, sandboxing & approval modes](/foundations/permissions/) for scoping and gating at the boundary, and [Configuration & profiles](/foundations/configuration/) for the config-entry approach to agent reach.
