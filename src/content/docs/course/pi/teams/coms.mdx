...
description: "Let two leads talk directly instead of routing through you, with the same four-verb protocol either way - then run the actual test for whether a peer channel is earning its keep or just a habit."
sidebar:
...
What the coms extension you've been building against actually ships is a local transport: a small socket per agent (a unix socket, or a named pipe on Windows), discovered through a per-project registry of small files the extension itself writes and prunes. That's enough for leads on the same machine - which covers everything `stash` needs. Leads on separate machines or in separate sandboxes are a real extension of the same four-verb idiom, but not a mode this same extension quietly switches into - it's a *different* extension, with its own dedicated hub process to run and secure and its own distinct tool names, standing in for the sockets. That's real infrastructure, not a free upgrade. Don't reach for it until you actually have leads that can't share a filesystem.
