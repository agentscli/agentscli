...
That gap - between the reading a job takes and the reading you actually need back - is exactly what a subagent closes. It's a separate session with its own context window that does the scoped job off to the side and reports back, so the noisy intermediate reads stay in the child rather than landing in your thread.{/* VERIFY: subagent-payload-shape */} OpenCode ships three of them ready to use, no setup, invoked by `@mention` right in your prompt.
