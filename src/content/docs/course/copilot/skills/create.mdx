...
description: "Author the audit-event convention as a SKILL.md - a folder, a markdown file, two required frontmatter fields, and any scripts the procedure needs alongside it."
sidebar:
...
:::tip[Try it]
Write a `SKILL.md` for a repeatable procedure in your repo - the audit-event convention above is a good model. Start a new chat and describe the situation naturally, without naming the skill. If it doesn't fire, revise the `description` field until Copilot picks it up on its own; that one line is the whole trigger mechanism.
:::
