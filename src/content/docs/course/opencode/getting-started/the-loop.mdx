...
For your very first turn on an inherited codebase, start in `plan`. Tab over to it before you type anything. The point isn't that you're planning a feature - it's that `plan` is gated by construction, so you get to watch the agent gather context with the writing hand tied behind its back. There is no "trust me" required: nothing lands without you clearing it, because that's the agent's own configuration, not your vigilance.
