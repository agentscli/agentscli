...
Those lifecycle events, in the concrete: each tool's actual event names, where they sit in a session, and which ones can block the action they fire on. Click any event for what it fires on and a working config.
...
| Deterministic gate (allow/deny) | Yes (`PreToolUse`) | Yes (`PreToolUse`, exit 2) | Yes (plugin `permission.ask`) | Yes (`"permission": "allow" \| "deny" \| "ask"`) | Yes (`preToolUse`, where supported) | Yes (extension can intercept and block) |
