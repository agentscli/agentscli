...
Here's the claim this lesson proves, and it'll look strange until you've seen why: the fix from last chapter replaced an entire function - four lines longer than the actual bug - when only one line was wrong. That wasn't Pi being sloppy, and it wasn't being thorough for its own sake. It was the direct, mechanical consequence of one rule buried in a single tool's description. By the end of this lesson you'll be able to point at that rule and explain exactly why a smaller edit would have failed outright.
...
Line counts and pass/fail counts in `stash`'s transcripts (312 lines, 12 passed, and so on) are invented for this course's running example - nobody has run this exact session against a real repo. Pi's own limits, like the one a few sections down for `bash`, are quoted straight from its source, not estimated. Where a number matters, this module tells you to count it yourself rather than take it from the page.
:::

If that file had been ten times longer, you'd have seen a truncation notice instead of a clean read - and the very next tool call would have been another `read` at the offset it handed back. Nothing hidden, nothing magic - just a stated boundary and a stated way past it. **A read either gives you the whole file or tells you exactly where it stopped.**
...
Now predict the diff before you look at it. The bug was that `parse_page_date()` tried exactly one fixed date format and returned `None` silently the moment a date didn't match it. The one buggy line is `return datetime.strptime(raw, "%B %d, %Y").date()`. That's a common enough shape of Python that, in a file with several hundred lines and more than one date-handling helper, it isn't safe to assume it's unique on sight - and `edit` doesn't get to assume it either. If the model had sent that bare line alone as `oldText`, and the file happened to contain anything close enough to it elsewhere, `edit` would have refused rather than guess. Now look at what `oldText` actually contains:
...
The whole function signature and body, not just the one changed line. That's the payoff: the extra lines in that diff aren't padding, and they aren't the model being cautious for style points. They're exactly what turns an ambiguous fragment into the unique, non-overlapping match `edit` requires. The edit landed clean on the first try because the match was unambiguous by construction. If it hadn't been, you'd have seen the tool refuse and the model retry with more surrounding text - not silently touch the wrong occurrence.

The output you get back isn't just "done," either - `edit` returns a real diff for display and a standard patch underneath it, plus the line number where the change starts. That's what let the TUI show you a clean before/after instead of a vague confirmation. **The size of an edit's `oldText` is a uniqueness proof, not a style choice.**

## `write` - the blunt instrument, on purpose

`write` takes a path and full file content, and it does exactly one thing: create the file (making any parent directories along the way) or overwrite it wholesale. No merge, no partial application, no matching logic at all - the opposite end of the spectrum from `edit`'s precision. That's why the system prompt steers the model toward `write` only for brand-new files or genuine full rewrites, and toward `edit` for anything that's touching part of an existing one. You didn't see `write` fire during the date fix, and that's the expected outcome - a targeted bug fix in an existing function is `edit`'s job, not `write`'s. **A tool that can't merge is a tool that can't corrupt a merge, either.**
...
Output is capped too - the **last 100 lines or 10KB**, whichever limit hits first, the same truncation helper `read` uses, called here with its own limits for this tool. {/* corpus: research/pi/02-architecture-agent-loop.md, §3 bash - DEFAULT_MAX_LINES/DEFAULT_MAX_BYTES verified at 100 lines / 10KB for bash specifically; read's own constants are unverified in the corpus, so no number is stated for read above. */} Here's the part worth remembering: when a command's output gets truncated, the *full* output isn't thrown away - it's written to a temp file, and the path comes back in the result so the model can `read` or grep the rest itself if it needs to:
...
That's what ran after the edit landed - the fix verifying itself against `stash`'s existing test suite, not you taking the model's word for it. If those tests had produced a wall of output past the cap, nothing would have been lost; it would just be sitting in a temp file, one `read` call away. **A capped tool and a lossy tool are not the same thing.**
...
:::tip[Try it]
Ask Pi to read a specific file in stash, then edit one line, then run the tests. Watch the tool calls scroll by - read, edit, bash - and count how many the fix took. Now ask it to do the same thing "in one step" and notice it still uses the same three tools; there is no shortcut that skips the set.
:::
