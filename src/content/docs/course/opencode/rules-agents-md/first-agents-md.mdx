...
One file at the project root is the simple case. Real repos have subdirectories, and OpenCode's discovery walks *up* from the working directory until it finds the first matching project instruction file. A nested `AGENTS.md` therefore wins over an ancestor `AGENTS.md`; the files are not all stacked automatically. If you need a deliberate additive set, list the files in `opencode.json`'s `instructions` field. [Next: discovery and the nesting walk](/course/opencode/rules-agents-md/discovery-and-nesting/).
