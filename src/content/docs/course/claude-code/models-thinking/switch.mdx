...
description: "A copy tweak doesn't need your most capable model. Drop a gear with /model - the picker, session versus default, and when a heavier model earns its cost."
sidebar:
...
:::tip[Try it]
Give Claude a mechanical task (rename a variable across files) on the default model. Then switch to a stronger model with `/model` and ask a design question about the same codebase. Notice the difference in response depth.
:::
