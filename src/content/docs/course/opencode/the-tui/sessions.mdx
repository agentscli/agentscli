...
:::tip[Try it]
Start a session, make a change, exit. Resume with `--continue` and ask what you did last. Then list sessions with `opencode session list` and resume a different one by ID. Notice what context survived.
:::

Sessions pile up turns, and eventually a thread gets long enough that the context window starts to strain - or you make a change you wish you hadn't. [Next: undo, redo, and compaction](/course/opencode/the-tui/undo-redo-compact/). {/* VERIFY: undo-redo-slug-resolves */}
